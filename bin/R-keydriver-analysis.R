...
configureNodeVisualization <- function(allnodes, signature, kdaMatrix, bNodeSz = 40, bFontSz = 12) {
  ...
  xcat2 <- c("NSIG_GKD", "NSIG_LKD", "NSIG_NKD")
  ...
  sig_status <- rep("NSIG", no.nodes)
  names(sig_status) <- allnodes
  sig_status[sigInNet] <- "SIG"
  kdr_status <- rep("NKD", no.nodes)
  names(kdr_status) <- allnodes
...
degreeByLinkPairs <- function(linkpairs, directed = F, cleangarbage = F) {
  ...
  if (directed) {
    ...
    iolinks <- mergeTwoMatricesByKeepAllPrimary(
      primaryMatrix = cbind(uniquenames),
...
downStreamGenes <- function(netpairs, seednodes, N = 100, directed = TRUE) {
  ...
  cnt <- N
  while (T) {
    retlinks <- findNLayerNeighborsLinkPairs(
      linkpairs = netpairs, subnetNodes = prenodes,
...
keyDriverAnalysis <- function(inputnetwork, signature, directed = T, nlayer_expansion = 1,
  ...
  if (length(wholeOvlp) <= 2) {
    return(NULL)
  }

  if (nlayer_expansion >= 1) {
    # expand network by n-layer nearest neighbors
    expandNet <- findNLayerNeighborsLinkPairs(
      linkpairs = inputnetwork,
  ...
  if (is.null(expandNet)) {
    return(NULL)
  }
  if (dim(expandNet)[1] <= 10) {
    return(NULL)
  }
  ...
  if (directed) {
    ret <- keydriverInSubnetwork(
      linkpairs = expandNetIdx, signature = signatIdx, background = c(no.wholenodes, no.wholeOvlp),
...
keydriverInSubnetwork <- function(linkpairs, signature, background = NULL, directed = T, nlayers = 6,
  ...
  for (i in c(1:no.subnetsize)) {
    ...
    res <- c(
      min_nohits, min_noidn, no.overlapped, no.subnetsize, background2[2], background2[1],
  ...
  colnames(kdMatrix) <- c(
    "hits", "downstream", "signature_in_subnetwork", "subnetwork_size",
  ...
  if (boost_hubs) {
    if (!network_as_signature) {
      ...
      colnames(cutmatrix) <- c(
        "mean_downstream", "sd_downstream", "enrichedNodes_cut",
      ...
      if (directed) {
        ...
        cutmatrix <- rbind(c(
          mean(no.dsnodes), sd(no.dsnodes), concatenate(mymincut, ";"),
      ...
      colnames(cutmatrix) <- c(
        "mean_downstream", "sd_downstream", "cut_downstream",
  ...
  colnames(fkd) <- c(
    "keydrivers", "is_signature", "hits", "downstream", "signature_in_subnetwork",
  ...
  degreeAll <- cbind(allnodes, no.dsnodes)
  colnames(degreeAll) <- c("node", "downstream")
...
mergeTwoMatricesByKeepAllPrimary <- function(primaryMatrix, minorMatrix, missinglabel = "",
  ...
  if (is.null(keepAllPrimary)) { # normal merge: to have the common elements
    myMatrix <- merge(mustbeGenesMatrix, minorMatrix,
      by.x = 1, by.y = 1, all.x = FALSE,
...
for (em in modules) {
  ...
  if (layer >= 1) {
    ...
    expandNet <- findNLayerNeighborsLinkPairs(
      linkpairs = cnet, subnetNodes = genes,
  ...
  if (directed) {
    ret <- keydriverInSubnetwork(
      linkpairs = expandNet, signature = genes, background = NULL, directed = directed,
      nlayers = layer, enrichedNodes_percent_cut = -1, FET_pvalue_cut = 0.05,
      boost_hubs = T, dynamic_search = T, bonferroni_correction = T, expanded_network_as_signature = F
    )
  } else {
    ret <- keydriverInSubnetwork(
      linkpairs = expandNet, signature = genes, directed = directed,
      nlayers = layer, enrichedNodes_percent_cut = -1, FET_pvalue_cut = 0.05,
      boost_hubs = T, dynamic_search = T, bonferroni_correction = T, expanded_network_as_signature = F
    )
  }
...
write.table(paraMatrix, xkdFpara,
